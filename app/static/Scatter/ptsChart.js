// TODO:
// orientation line show yellow for length of rally and green/red for
// winner/error
...
function ptsMatch() {
   ...
   function chart(selection) {
    ...
    chart.width = function(value) {
        ...
        // pts_charts.forEach(e => e.width(value));
    ...
    chart.height = function(value) {
        ...
        if (typeof update === 'function') update(true);
        // pts_charts.forEach(e => e.height(value));
    ...
    chart.update = function(resize) {
       ...
       sets.forEach(function(e, i) {
          ...
          pts_charts[i].options({ id: i, resize: options.resize });
...
function ptsChart() {
        ...
        selection.each(function () {
            ...
            update = function(resize) {
               ...
               var yScale = d3.scale.linear()
                    ...
                    .domain([-2, set_options.points.to_set - 1]);
               ...
               set_box.enter()
                ...
                var lineGen = d3.svg.line()
                   ...
                   .y(function(d) { return yScale(set_options.points.to_set - d.pts); })
                ...
                breakpoints
                   ...
                   .attr("cy", function(d) { return yScale(set_options.points.to_set - d.pts); })
                   .attr("r", 2)
               ...
               function displayScore(resize) {
                  // if set is complete, display set winner and score
               ...
               function calcStops(point, i) {
                  ...
                  if (options.display.win_err_highlight) {
                     ...
                     var rally_pct = rally ? 100 - Math.floor(rally.length / longest_rally * 100) : 0;
    ...
    function checkSequence(point) {
       var last_row = data.length ? data[data.length - 1] : { point: '0-0' };
       var last_point = typeof last_row == 'object' ? last_row.point : last_row;
       last_point = last_point.indexOf('G') >= 0 ? '0-0' : last_point;
       if (point.indexOf('T') >= 0 && options.set.tiebreak && chart.games().length >= options.set.games) {
    ...
    function determineWinner(point) {
       var last_point = data.length ? data[data.length - 1].point : '0-0';
       last_point = last_point.indexOf('G') >= 0 ? '0-0' : last_point;
    ...
    function determinePoint(winner) {
       var last_point = data.length ? data[data.length - 1].point : '0-0';
       last_point = last_point.indexOf('G') >= 0 ? '0-0' : last_point;
       var games = data.length ? data.map(function(m) { return m.point.indexOf('G') >= 0 ? 1 : 0; }).reduce(function(a, b){return a+b;}) : 0;
    ...
    function boxWidth() {
       ...
       if (player_data[0].length && (player_data[0].last().pts == 0 || player_data[1].last().pts == 0)) pw = dl;
       return pw;
    }

function rallyTree() {
   ...
   // this should only be a LOCAL behaviour
   //
   ...
   // functions which should be accessible via ACCESSORS
    ...
    function chart(selection) {
      ...
      update = function(opts) {
          ...
          maxRally = d3.max(points, function(point) { if (point.rally) return point.rally_length });
          if (maxRally < 3) {
          ...
          points.forEach(function(e, i) {
             e.i = i;
             if (ryl[e.winner][e.rally_length]) {
                ryl[e.winner][e.rally_length] += 1;
             } else {
                ryl[e.winner][e.rally_length] = 1;
             }
             e.rl = [ryl[0][e.rally_length], ryl[1][e.rally_length]];
          });
...
//              .attr('gen', function(d) { return aP(d); } )
   ...
   chart.reset = function(clear_active) {
    ...
    chart.points = function(values) {
       ...
       if ( values.constructor === Array ) {
          ...
          points.forEach(function(point) {
             var rally_length = point.rally ? point.rally.length : 0;
             if (point.result != 'Ace') rally_length += 1;
             point.rally_length = rally_length;
          });
       }
       return chart;
    }
    ...
    function shuffle(player, option) {
       ...
       order.push('Z');
       var maxRally = d3.max(points, function(point) { return point.rally_length });
       for (y=0; y <= maxRally; y++) {
          var rally_group = [];
          for (var i=0; i < points.length; i++) { 
             if (points[i].rally_length == y && points[i].winner == player) { rally_group.push(points[i]); }
          }

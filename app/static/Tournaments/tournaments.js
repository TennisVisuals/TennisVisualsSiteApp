!function() { 
   ...
   var te = {};
      ...
      var highlight_player = (okeys && okeys.indexOf('player') >= 0) ? decodeURIComponent(aip.QueryString['player']) : undefined;
      ...
      function bh() {
         ...
         .on('typeahead:selected', function(event) { go(); });
      }
      ...
      function loadTournament(id) {
         ...
         $.ajax({
            ...
            if (result.data) {
               ...
               data.draws.forEach(function(d) { fillDraw(d); });
               if (highlight_player) { highlightPlayer(highlight_player); }
               sendReport('TE: ' + id);
            }
            te.bh();
         });
         ...
         contexts.forEach(function(c) {
            c.draw.first_round.forEach(function(e, i) { 
               if (e.player) {
                  ...
                  e.players.forEach(function(p) { players.push(p.player); });
               } else {
                  console.log('Missing Player Name at index:', i);
               }
            });
         });
         return players.filter(function(item, i, self) { return self.lastIndexOf(item) == i; });
      }
      ...
      function fillDraw(results) {
         ...
         draws.forEach(function(r, i) {
            ...
            var seeds = contexts[c].draw.first_round.filter(function(f) { return f.seed; });
      ...
      function addOutcome(draw_nest, draw_walk, player_data, result) {
         ...
         Object.keys(result).forEach(function(e) { target[e] = result[e]; });
      }
   ...
   if (typeof define === "function" && define.amd) define(te); else if (typeof module === "object" && module.exports) module.exports = te;
   this.te = te;

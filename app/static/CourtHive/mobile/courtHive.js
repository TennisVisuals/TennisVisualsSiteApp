 ...
 */
   ...
   navigator.geolocation.getCurrentPosition(function(pos){ geoposition = pos; });
   ...
   function updateAppState() {
      Object.keys(settings).forEach(key => {
         let em = document.getElementById(key);
   ...
   function restoreAppState() {
      let app_settings = BrowserStorage.get('CH_AppSettings');
      if (app_settings) settings = JSON.parse(app_settings);
      Object.keys(settings).forEach(key => {
         let em = document.getElementById(key);
         if (em) em.checked = settings[key];
      });
      let app_state = BrowserStorage.get('CH_AppState');
   ...
   function modalExport(match_id = BrowserStorage.get('current_match'), what = 'match') {
      ...
      `;
      let modaltext = `<div>${header}${!device.isIDevice ? save : ''}${copy}</div>`;
      ...
      <div id='match_${match_id}' class='flexcenter mh_swipe swipe'>
         <div id='match_${match_id}_export' class='flexcols flexcenter mh_export' style='display: none;' onclick="modalExport('${match_id}')">
            <div class='mh_fullheight'> 
               <div class='export_action action_icon iconexport'></div> 
            </div>
         </div>
         ...
         <div id='match_${match_id}_delete' class='flexcols flexcenter mh_trash' style='display: none;' onclick="deleteMatch('${match_id}')">
            <div class='mh_fullheight'> 
               <div class='mh_action action_icon iconrecycle'></div> 
               </div>
         </div>
      </div>`
      return html;
   ...
   function loadMatch(match_id, view = 'entry') {
      ...
      let muid = match_data.match.muid;
   ...
   function statCounters() {
      ...
      if (stats && stats.teams) {
         ...
         stat_keys.forEach(key => {
            ...
            let table = ` <div class='statrow' id="${key}_chart" style='display:none'> <div class='action_icon iconchart'></div> </div> `;
            html += row + table;
         });
      }
   ...
   function editPlayer(index) {
      ...
      console.log(env.edit_player);
      document.getElementById('editplayer').style.display = 'flex';
      let player = match.metadata.players(env.edit_player);
   ...
   function broadCastScore(action) {
      let result = '';
      let score = match.score();
      let scoreboard = match.scoreboard(0);
      let players = match.metadata.players();
      if (action && action.point && action.point.result) {
         let point = action.point;
         result = [point.hand, point.stroke, point.result].filter(f=>f).join(' ');
      }
      if (app.broadcast) {
         // in the future no need to broadcast players/match data 
         // because that will be done when registering the match with scores app
         coms.socket.emit('match score', { match: match.metadata.defineMatch(), players, score, scoreboard, result, serving: match.nextTeamServing() });
      }
   }
   ...
   function modalShare(match_id) {
      ...
      let modaltext = ` <p> <h1>First Track a Match!</h1> </p> `;
      let tv_link;
      if (match_id) {
         let match_data = BrowserStorage.get(match_id);
         ...
         function shareResult(response) {
            ...
            if (!result.error) {
               ...
               let tvb = `TennisVisuals Match Visualization: ${tv_link}`;
               let broadcast_link = app.broadcast ? `CourtHive Live Score: ${location.origin}/scores?muid=${muid}` : '';
               let email_message = `${tvb}%0D%0A ${broadcast_link}%0D%0A `;
               let copy_message = `${tvb}\r\n${broadcast_link}\r\n`;
               modaltext = `
                  <p>&nbsp;</p>

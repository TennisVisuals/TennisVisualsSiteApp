...
(function() {
  d3.parsets = function() {
    ...
    function parsets(selection) {
      selection.each(function(data, i) {
                ...
                .on("drag", function(d) {
                  ...
                  for (var i = 1; i < dimensions.length; i++) {
                    if (height * dimensions[i].y < height * dimensions[i - 1].y) {
                      ...
                      // preserve new dimension sort
                      var dkeys = Object.keys(dimensions);
                      var pdim = [];
                      for (var f=0; f < dkeys.length; f++) {
                         pdim.push(dimensions[dkeys[f]].name)
                      }
                      pdimensions = pdim;
        ...
        function highlight(d, ancestors) {
           // d3.select('#container').selectAll('.bar').select('rect').style('fill', 'white');
          if (dragging) return;
          var highlight = [];
          (function recurse(d) {
            highlight.push(d);
        ...
        function updateCategories(g) {
          ...
          category
              ...
              .on("dblclick", p_scope_change)
              .on("click", p_ribbon_shots)
              .on("mouseout.parsets", function(d) { 
                 unhighlight;
              })
              .on("mousedown.parsets", cancelEvent)
              .call(d3.behavior.drag()
    ...
    // return d3.rebind(parsets, event, "on").value(1).width(550).height(385);
    return d3.rebind(parsets, event, "on").value(1).width(p_width).height(p_height);
  ...
  function defaultTooltip(d) {
    var count = d.count,
        ...
        counts = [];
    while (d.parent) {
      if (d.name) { path.unshift(d.name); counts.unshift(d.count); }
      d = d.parent;
    }
    // replaced original percent to give percent as portion of first dimension
    // rather than as portion of total shots
    // return path.join(" → ") + "<br>" + comma(count) + " (" + percent(count / d.count) + ")";
    return path.join(" → ") + "<br>" + comma(count) + " (" + percent(count / counts[0]) + ")";
  }

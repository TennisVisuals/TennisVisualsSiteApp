function scatterPlot() {
    ...
    var events = {
       ...
       'voronoi': { 'mouseover': showTooltip, 'mouseout': removeTooltip }
    };
    ...
    function chart(selection) {
        selection.each(function () {
            ...
            var clippath = root.append('defs').append('clippath').attr('id', 'global-cut-off');
            ...
            update = function(resize) {
                ...
                update_circleGroups
                  ...
                  .transition("updateCircles").duration(transition_time)
                  ...
                  .attr("r", function(d) { 
                     return (options.display.highlight.radius && highlight && checkAttributes(d)) ? 
                             options.display.highlight.radius :rScale(d[options.data.r_scale]); 
                  })

                function checkAttributes(d) {
                   var attributes = options.display.highlight.data;
                   if (!attributes.length) return false;
                   for (a=0; a < attributes.length; a++) {
                      var attribute = attributes[a];
                      if (d[options.data[attribute]].toLowerCase().indexOf(highlight.toLowerCase()) >= 0) return true;
                   }
                   return false;
                }

                //Define a clip-path which is the union of all interactive zones
                d3.select("#global-cut-off")
                  .selectAll("circle")
                   .data(_data_set)
                   .enter().append("circle")
                     .attr("cx", function(d) {return xScale(d[options.data.x]);})
                     .attr("cy", function(d) {return yScale(d[options.data.y]);})
                     .attr("r", options.datapoints.voronoiDistance);
                     
                //Apply the clip-path to the voronoi layer
                voronoiGroup.style("clip-path", "url(#global-cut-off)");
                ...
                voronoiGroup.selectAll(".voronoi" + options.id).remove();

                voronoiGroup.selectAll(".voronoi" + options.id)
                   .data(voronoi(_data_set))
                  .enter()
                   .append("path")
                   .attr("id", function(d, i) { return "voronoi" + options.id + i; })
                   .attr('gen', function(d, i) { return genPath(d, i); } )

                function genPath(d, i) {
                   if (!d) return;
                   d3.select('#voronoi' + options.id + i)
                     .datum(d)
                     .style("stroke", function(f) { return options.display.voronoi ? "#2074A0" : undefined })
                     .attr("class", function(d) { return "voronoi" + options.id + " A" + i; }) 
                     .style("fill", "none")
                     .style("pointer-events", "all")
                     // .on('mouseover', function(d) { 
                     .on('mouseenter', function(d) { 
                        if (!chosen_group || d.point[options.data.group] == chosen_group) {
                           if (events.voronoi.mouseover) { 
                              var target = d3.select(".A" + i);
                              events.voronoi.mouseover(d.point, i, target) 
                           };
                     ...
                     .on('mouseout', function(d) { 
                        ...
                        if (events.voronoi.mouseout) { 
                           var target = d3.select(".A" + i);
                           events.voronoi.mouseout(d.point, i, target); 
                        };
                     })
            ...
            function showCrossHairs(d, i) {
               ...
               chartWrapper.append("g")
                  ...
                  .append("line")
                     ...
                     .transition('crosshairX').duration(200)
                     .style("opacity", 0.5);
               ...
               chartWrapper.append("g")
                  ...
                  .append("line")
                     ...
                     .style("opacity",  0)
                     .style("pointer-events", "none")
                     .transition('crosshairY').duration(200)
                     .style("opacity", 0.5);
            }
               ...
               return function(d, i) {
                  ...
                  chartWrapper.selectAll(".data_rows" + options.id)
                     ...
                     .style("opacity", function(d) {
                        return d[options.data.group] != chosen ? opacity : 1;
                     });
                 };
            }

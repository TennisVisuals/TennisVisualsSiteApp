...
function burstChart() {
    ...
    function chart(selection) {
        selection.each(function () {
            ...
            var burstG = root.append('g').attr('class', 'burstG');
            nodes.burst = burstG.append('g');
            nodes.hover = burstG.append('g');
            ...
            update = function(opts) {
               ...
               if (options.display.sizeToFit || (opts && opts.sizeToFit)) {
                  ...
                  burstG.attr("transform", "scale(" + scale_width + "," + scale_width + ")");
               }
    ...
    function displayFreeze(d) {
       var whom = d && d.player ? d.player : '';
       if (d && d.score) whom += ' : ' + d.score;
       if (aip) aip.sendReport('Burst:' + whom);
       frozen = 1 - frozen;
       ...
       if (nodes.burst) {
          ...
          nodes.burst.selectAll('path')
             .attr('opacity', function(d) { 
                if (!player) return 1;
                if (d.player && d.player.toLowerCase() == player.toLowerCase()) highlighted.push(d);
